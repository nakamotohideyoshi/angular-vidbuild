...
export class VidPlayerComponent implements OnDestroy, OnInit {
  ...
  @Input() project: vbuild.Project;
  @Input() public videoSrc: string;
  @Output() public frameSeek: EventEmitter<number> = new EventEmitter();
  @Output() public durationChange: EventEmitter<number> = new EventEmitter();
  ...
  public onDurationChange(evt: Event): void {
    this.durationChange.emit(Number((<HTMLSelectElement>evt.target).value));
  }
